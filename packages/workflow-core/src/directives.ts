import { GraphicDirectiveSchema, graphicGrammars, type AspectPresetId } from '@agl/composition-schema';
...
function paragraphIndexAt(articleText: string, offset: number) {
  const before = articleText.slice(0, offset).trim();
  if (!before) return 0;
  return before.split(/\n\s*\n/g).filter(Boolean).length;
}

function surroundingExcerpt(articleText: string, paragraphIndex: number) {
...
function parseYamlish(body: string) {
  ...
  for (const rawLine of lines) {
    ...
    if (listMatch && listKey) {
      (out[listKey] as string[]).push(parseScalar(listMatch[1]));
      continue;
    }
...
function parseDirectiveBody(body: string) {
  ...
  if (trimmed.startsWith(':')) return { description: trimmed.slice(1).trim() };
  if (trimmed.startsWith('{')) return JSON.parse(trimmed);
  return parseYamlish(trimmed);
}

function normalizeDirective(parsed: Record<string, unknown>, raw: string, start: number, end: number, paragraphIndex: number): ExtractedGraphicDirective {
  const candidate = {
  ...
  if (!result.success) {
    ...
    return {
      ...
      type: graphicGrammars.includes(candidate.type as never) ? String(candidate.type) : 'metaphor',
      placement: 'replace',
...
export function extractGraphicDirectives(articleText: string): ExtractedGraphicDirective[] {
  const directives: ExtractedGraphicDirective[] = [];
...
export function stripGraphicDirectives(articleText: string) {
  return articleText.replace(DIRECTIVE_RE, '').replace(/\n{3,}/g, '\n\n').trim();
}

export function inferTitle(articleText: string) {
  return articleText.split(/\r?\n/).find(line => /^#\s+/.test(line))?.replace(/^#\s+/, '').trim() || 'Visualized article';
}

export function directiveToSpot(directive: ExtractedGraphicDirective, articleText: string, priority = 1) {
